...
| # | Grupo | Ruta base | Rol | Endpoints |
|---|-------|-----------|-----|-----------|
| 1 | [Autenticación / Sesión](#1-autenticación--sesión) | `/api/auth` | Público (login/refresh/logout), autenticado (me) | 4 |
| 2 | [Socios](#2-socios) | `/api/members` | Lectura: autenticado · Escritura: Administrador | 6 |
| 3 | [Puestos](#3-puestos) | `/api/stalls` | Lectura: autenticado · Escritura: Administrador | 6 |
| 4 | [Servicios cobrables](#4-servicios-cobrables) | `/api/services` | Lectura: autenticado · Escritura: Administrador | 6 |
| 5 | [Giros comerciales](#5-giros-comerciales) | `/api/business-types` | Lectura: autenticado · Escritura: Administrador | 5 |
| 6 | [Bancos](#6-bancos) | `/api/banks` | Lectura: autenticado · Escritura: Administrador | 6 |
| 7 | [Proveedores](#7-proveedores) | `/api/providers` | Lectura: autenticado · Escritura: Administrador | 6 |
| 8 | [Cuentas por cobrar](#8-cuentas-por-cobrar) | `/api/account-receivables` | Autenticado (excepto exonerar: Operador de Caja) | 6 |
| 9 | [Lecturas de consumo](#9-lecturas-de-consumo) | `/api/consumption-readings` | Autenticado | 3 |
| 10 | [Pagos](#10-pagos) | `/api/payments` | Operador de Caja | 3 |
| 11 | [Ingresos externos](#11-ingresos-externos) | `/api/incomes` | Operador de Caja | 3 |
| 12 | [Egresos](#12-egresos) | `/api/expenses` | Operador de Caja | 6 |
| 13 | [Canjes bancarios](#13-canjes-bancarios) | `/api/bank-exchanges` | Operador de Caja | 3 |
| 14 | [Reportes](#14-reportes) | `/api/reports` | Autenticado | 7 |
| 15 | [Catálogos de solo lectura](#15-catálogos-de-solo-lectura) | varios | Autenticado | 16 |
...
### `PATCH /api/members/{uuid}/activate` **\[NUEVO\]**

- **Tipo:** PATCH · **Headers:** `Authorization: Bearer <token>`
- **Rol:** Administrador.
- **Body:** ninguno.
- **Respuesta exitosa — `200 OK`**: `MemberResponse` con `active: true` (reactivación).
- **Errores:** `404`, `409`, `401`, `403`.
...
### `PATCH /api/stalls/{uuid}/activate` **\[NUEVO\]**

- **Tipo:** PATCH · **Headers:** `Authorization: Bearer <token>`
- **Rol:** Administrador.
- **Body:** ninguno.
- **Respuesta exitosa — `200 OK`**: `StallResponse` con `active: true` (reactivación).
- **Errores:** `404`, `409`, `401`, `403`.
...
### `PATCH /api/services/{uuid}/activate` **\[NUEVO\]**

- **Tipo:** PATCH · **Headers:** `Authorization: Bearer <token>`
- **Rol:** Administrador.
- **Body:** ninguno.
- **Respuesta exitosa — `200 OK`**: `ServiceResponse` con `active: true` (reactivación).
- **Errores:** `404`, `409`, `401`, `403`.
...
### `PATCH /api/banks/{uuid}/activate` **\[NUEVO\]**

- **Tipo:** PATCH · **Headers:** `Authorization: Bearer <token>`
- **Rol:** Administrador.
- **Body:** ninguno.
- **Respuesta exitosa — `200 OK`**: `BankResponse` con `active: true` (reactivación).
- **Errores:** `404`, `409`, `401`, `403`.
...
### `PATCH /api/providers/{uuid}/activate` **\[NUEVO\]**

- **Tipo:** PATCH · **Headers:** `Authorization: Bearer <token>`
- **Rol:** Administrador.
- **Body:** ninguno.
- **Respuesta exitosa — `200 OK`**: `ProviderResponse` con `active: true` (reactivación).
- **Errores:** `404`, `409`, `401`, `403`.
...
> Nota: este path (`/summary`) debe declararse antes que `/api/account-receivables/{uuid}`
